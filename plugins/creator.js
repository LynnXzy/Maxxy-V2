...
if (command == 'creator') {
  ...
  const sentMsg = await conn.sendContactArray(m.chat, [
    [`${nomorown}`, `${await conn.getName(nomorown+'@s.whatsapp.net')}`, `👑 Developer Bot `, `🚫 Don't call me 🥺`, `botzlynx@gmail.com`, `🄼🅈 Malaysia`, `🚀 https://Frezzy-XD.github.io/`, `👤 Owner Gua Ganteng kok`],
    [`${conn.user.jid.split('@')[0]}`, `${await conn.getName(conn.user.jid)}`, `🔥 Bot WhatsApp 🐣`, `📵 Don't spam/call me 😢`, `Nothing`, `🄼🅈 Malaysia`, `🚀 https://lynk.id/botzlynn_/`, `🤖 Hanya bot biasa yang kadang suka eror ☺`]
  ], fakes)
